...
-- CONTEÚDO ABORDADO NO CURSO:
	-- Operações DDL: Criação e remoção de bancos e tabelas;
	-- Inclusão de dados em tabelas;
	-- Atualização de dados em tabelas;
	-- Exclusão de dados em tabelas;
	-- Operações matemáticas com dados das tabelas;
	-- Criação Primary-Key em uma tabela existente;
	-- Realização de consultas com base em filtros simples;
	-- Utilização de operadores de comparação para filtar consultas;
	-- Utilização de filtros com base em datas para realizar consultas;
	-- Utilização de operadores lógicos AND e OR para filtrar consultas.
...
-- FAÇA UMA SELEÇÃO DOS PRODUTOS QUE TENHAM VALOR MAIOR OU IGUAL A R$ 10,00.
SELECT * FROM [TABELA DE PRODUTOS] WHERE [PRECO DE LISTA] >= 10.0

-- FAÇA UMA SELEÇÃO SEMELHANTE A ANTERIOR MAS DESSA VEZ OS CAMPOS EXIBIDOS DEVEM SER APENAS 'CODIGO DO PRODUTO' E 'PRECO DE LISTA'.
SELECT [CODIGO DO PRODUTO] AS 'SKU', [PRECO DE LISTA] AS 'PREÇO' FROM [TABELA DE PRODUTOS] WHERE [PRECO DE LISTA] >= 10.0

-- AGORA FILTRE APENAS OS PRODUTOS COM PREÇO MENOR QUE R$ 9,00
SELECT * FROM [TABELA DE PRODUTOS] WHERE [PRECO DE LISTA] < 9.00

-- AULA 08: FILTRANDO POR DATAS
-- FAÇA UMA SELEÇÃO DOS CLIENTES QUE NASCERAM A PARTIR DE 1995.
SELECT * FROM [TABELA DE CLIENTES] WHERE [DATA DE NASCIMENTO] >= '1995-01-01'

-- FAÇA UMA SELEÇÃO DOS CLIENTES QUE NASCERAM NO ANO DE 1995.
SELECT * FROM [TABELA DE CLIENTES] WHERE YEAR([DATA DE NASCIMENTO]) = 1995

-- AGORA FILTRE OS CLIENTES QUE NASCERAM NO MÊS DE SETEMBRO, INDEPENDENTEMENTE DO ANO.
SELECT * FROM [TABELA DE CLIENTES] WHERE MONTH([DATA DE NASCIMENTO]) = 9

-- AULA 10: FILTROS COMPOSTOS
-- SELECIONE OS CLIENTES DA CIDADE DO RIO DE JANEIRO QUE TENHAM MAIS DE 22 ANOS. A EXIBIÇÃO DEVE INCLUIR APENAS NOME, CIDADE E IDADE.
SELECT [NOME], [CIDADE], [IDADE] FROM [TABELA DE CLIENTES] WHERE [CIDADE] = 'Rio de Janeiro' and [IDADE] > 22

-- AGORA FAÇA UMA SELEÇÃO QUE EXIBA PESSOAS QUE SEJAM DO SEXO FEMININO OU TENHAM NASCIDO NO ANO DE 2000.
