...
SELECT ESTADO, COUNT(NOME) FROM TABELA_DE_CLIENTES GROUP BY ESTADO;
...
SELECT CODIGO_DO_PRODUTO, SUM(QUANTIDADE) AS 'TOTAL_VENDIDO' FROM ITENS_NOTAS_FISCAIS GROUP BY CODIGO_DO_PRODUTO ORDER BY TOTAL_VENDIDO DESC;
-- CALCULANDO E EXIBINDO A MÉDIA DE PREÇOS DOS PRODUTOS AGRUPADAS POR TIPO DE EMBALAGEM.
SELECT EMBALAGEM, AVG(PRECO_DE_LISTA) AS 'PRECO_MEDIO' FROM TABELA_DE_PRODUTOS GROUP BY EMBALAGEM ORDER BY PRECO_MEDIO;
