...
-- REALIZANDO UMA SUBCONSULTA JUNTO À CLÁUSULA'IN' PARA DESCOBRIR QUAIS BAIRROS DE CLIENTES ESTÃO CONTIDOS NA LISTA DE BAIRROS DE VENDEDORES.
SELECT * FROM TABELA_DE_CLIENTES WHERE BAIRRO IN (SELECT DISTINCT BAIRRO FROM TABELA_DE_VENDEDORES)

-- USANDO SUBCONSULTAS PARA SUBSTITUIR O 'HAVING'.
-- Vejamos primeiro a consulta com 'HAVING'
SELECT EMBALAGEM, SUM(PRECO_DE_LISTA) AS PRECO_TOTAL
FROM TABELA_DE_PRODUTOS
GROUP BY EMBALAGEM
HAVING SUM(PRECO_DE_LISTA) >= 80

-- Agora, vejamos a consulta utilizando subconsulta.
SELECT SOMA_EMBALAGENS.EMBALAGEM, SOMA_EMBALAGENS.PRECO_TOTAL
FROM
(SELECT EMBALAGEM, SUM(PRECO_DE_LISTA) AS PRECO_TOTAL
FROM TABELA_DE_PRODUTOSnull
GROUP BY EMBALAGEM) SOMA_EMBALAGENS
WHERE SOMA_EMBALAGENS.PRECO_TOTAL >= 80null
